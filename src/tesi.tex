...
- Funzione \code{geom\_bar} riga 63. Genera un "diagramma a barre" utilizzando 
alcune funzioni definite in cplint\_r. Poichè abbiamo 7 elementi fissi sulle
ascisse dobbiamo usare \code{numlist(1,7,X)}, una lista da e 
\code{breaks=seq(0,7,1)}.
Inoltre \code{breaks=seq(0,max(df\$val),2)} per imitare il comportamento della 
versione c3js. CHECK.
