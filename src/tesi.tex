...
di organizzare i dati sotto forma matriciale. ... [TODO]
I data frame in R sono definiti come specializzazione della classe lista.
Rispetto ad una lista, infatti, esistono delle restrizioni... [TODO]

I data frame si rivelano anche efficienti per R stesso.... cite ... [TODO]
\cite{rDataFrame}
...
rserve\_client è una libreria Prolog che ha lo scopo di rendere possibile 
...
\begin{enumerate}
    \item Calcolo della matrice covarianza \texttt{K} dei punti noti.
    \item Inversa di \texttt{K}, \texttt{K\^-1}
    \item Calcolo di \texttt{K*}, che rappresenta la covarianza tra
          il punto x* (con y* incognita) e gli altri punti (x,y) noti.
    \item Trasposta di \texttt{K*} \texttt{K*T}
    \item \texttt{K**}, una matrice da un elemento che corrisponde alla 
          covarianza (x*,x*).
    \item  varianza = K** - (K* x K\^-1 x K*T)
    \item Esegui punto 2 -> 6  per tutti gli altri x* (punti (x*,y*) con y* 
          incognita,, i restanti 9 punti). Questo viene fatto ricorsivamente 
          usando sottoinsiemi delle liste inizali passate come argomento.
\end{enumerate}
Queste operazioni sono riportate nel listato~\ref{lst:grpRVarianceComputation}.
...
Anche le error bar sono una cosa che non c'era nella versione C3.js ma c'e' 
nell'articolo ... [TODO]
...
%quadrata della varianza, ad esempio: 
%\texttt{ymin=c(df1\$y-(1.96*sqrt(df4\$y)))}
...
[TODO: commented code snippets, not whole code]
...
distribuzioni di densità degli stati.
...
FOTO esempi versione c3js e R. [TODO]
