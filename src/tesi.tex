...
rserve-sandbox è una versiona di Rserve che gira in un contenitore Docker 
[TODO].
...
Non viene mai esposto un socket \emph{TCP/IP} ma solo un socket di tipo 
\emph{UNIX} [EXPLAIN DIFFERENCE TO SHOW THE ADDED 
SECURITY][FIXME] \cite{unixSocket}.  Questo è possibile attraverso il
file di configurazione [TODO]:
\texttt{https://github.com/frnmst/rserve-sandbox/blob/distro-package/Rserv.conf}
che viene letto da
\texttt{https://github.com/frnmst/rserve-sandbox/blob/distro-package/Rserv.sh}


Tutto lo schema viene spiegato nella figura~\ref{fig:rserveSandboxScheme}, 
mentre il suo file di configurazione si trova nel 
listato~\label{lst:rservconf}.
...
\begin{code}
    \caption{File di configurazione di rserve sandbox}
