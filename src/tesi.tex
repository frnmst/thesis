...
visualizzazione grafica. Con il termine R si definisce l'interprete
...
funzioni utili non comprese in R stesso~\footnote{Da questo momento in poi, 
per semplicità, si userà il termine R per indicare anche il linguaggio.}
[TODO: cite this].
...
però viene preferita la notazione \mint{r}|<-| piuttosto che il classico 
\mint{r}|=| Nel primo caso infatti non bisogna preoccuparsi di alcuni dettagli 
funzionali \cite{assignmentOperatorInR} \cite{rAssignmentOperator}

Una delle funzioni fondamentali di R è la funzione \mint{r}|c()|
Questa serve per concatenare un insieme arbitrario di valori, in modo che 
questi possano essere salvati all'interno di un'unica variabile. [TODO: cite 
concatenating lists; cite Vectors and assignments; from 
...
% Report and comment some of QVM's content.
...
.. Brief APT + PPA descriptiion ... [TODO]
