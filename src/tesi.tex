...
\cite{ggplot2} e 
https://books.google.it/books?id=XSekDgAAQBAJ\&pg=PT172\&lpg=PT172\&dq=%22ggplot2+is+a+plotting+system+for+R,+based+on+the+grammar+of+graphics,+which+tries+to+take+the+good+parts+of+base+and+lattice+graphics+and+none+of+the+bad+parts.%22\&source=bl\&ots=wSPaYoMAIO\&sig=Fx\_gTNTn1Zu5DQc1bElRctmtwBc\&hl=en\&sa=X\&ved=2ahUKEwjMpobchJLdAhUKFXwKHdB-BzQQ6AEwB3oECAMQAQ#v=onepage\&q=%22ggplot2%20is%20a%20plotting%20system%20for%20R%2C%20based%20on%20the%20grammar%20of%20graphics%2C%20which%20tries%20to%20take%20the%20good%20parts%20of%20base%20and%20lattice%20graphics%20and%20none%20of%20the%20bad%20parts.%22\&f=false
...
comando \mint{r}|<- c(<qualcosa>)|, dove "c" serve a concatenare un 
insieme di valori. \cite{rAssignmentOperator}
...
La libreria rserve\_client\cite{rserveclient} permette di utilizzare i comandi 
disponibili dell'interprete R direttamente da SWISH.
%%% poiché il supporto per R 
%%% all'interno di SWISH è ancora di recente implementazione. 
%%% Di solito quindi la parte computazionale viene affidata all'interprete 
%%% Prolog, mentre la parte grafica viene gestita dalla libreria c3js. Con 
%%% questa libreria invece sia la parte grafica sia quella computazionale sono 
%%% eseguite da R e visualizzate con SWISH.][DELETE THIS]
...
Grazie ad questa libreria, inoltre, è possibile usare direttamente comandi di R 
facendo minime modifiche rispetto allo stesso codice usato nell'interprete R.
...
Per poter utilizzare rserve\_client è anche necessario un server.
Rserve è un server TCP/IP che permette ad altri programmi di 
...
\emph{linkarne}[FIXME] la libreria \cite{rserve}.

rserve-sandbox è una versione di Rserve che gira all'interno di un sistema di 
virtualizzazione creato appositamente per lo scopo.
[TODO].
Per migliorare la sicurezza non viene mai esposto un socket \emph{TCP/IP} ma 
solo un socket di tipo \emph{UNIX} [EXPLAIN DIFFERENCE TO SHOW THE ADDED 
SECURITY][FIXME] \cite{unixSocket}.  
Questo è possibile attraverso il file di configurazione nel listato~\ref{lst:rservconf}.
...
%%%%
    ...
    \section{Docker}
\emph{Docker} è un implementazione di una virtualizzazione [O PSEUDO][FIXME] a 
livello di sistema 
...
rete più noti.
    ...
    \subsection{Configurazione di Docker per rserve-sandbox}

Per l'utilizzo dell'ambiente R in rserve-sandbox [DESCRIBE] è necessario 
l'interprete R ed un ambiente Docker configurato opportunamente.
...
direttamente legato ad rserve. [TODO AND/OR MOVEME?] [...]

Con il comando \texttt{CMD /bin/bash Rserv.sh}, presente nel Dockerfile,
Docker chiama 
\texttt{https://github.com/frnmst/rserve-sandbox/blob/distro-package/Rserv.sh}
che legge il file di configurazione di Rserve (vedere il listato 
~\ref{lst:rservconf}).
