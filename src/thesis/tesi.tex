...
\usepackage[backend=biber,sorting=none,style=trad-plain]{biblatex}
...
\usepackage{graphicx,url,amsfonts,epsfig,float,listings,subcaption}
%\usepackage{float}
%\floatstyle{boxed} 
%\restylefloat{figure}
\usepackage{caption}
...
Rserve sia integrato opportunamente con Docker. Il dockerfile che si 
è utilizzato contiene infatti le instruzioni per installare e avviare Rserve.
L'istruzione presente nel listato~\ref{lst:istruzioneInstallazionePacchettoR} 
consente di installare il pacchetto R ggplot2.

\begin{minipage}{\textwidth}
\centering
\begin{code}
    \caption{Istruzione installazione pacchetto R}
    \label{lst:istruzioneInstallazionePacchettoR}
    \begin{shellcode*}{}
...
\begin{itemize}
    ...
    \item  \texttt{-v /home/rserve:/home/rserve}, che serve a montare 
           la directory \texttt{/home/rserve} del sistema \emph{host} in quello 
           \emph{guest}.
...
guest. Per ottenere una directory condivisa fra la macchina virtuale e la 
macchina ospite è sufficiente aggiungere un'istruzione nel file 
\texttt{/etc/fstab} della macchina guest. Questo si può vedere nel 
listato~\ref{lst:comandoFstabQvm}.
...
Ora si può creare un backup dell'hard disk virtuale con l'opzione 
\mintinline{bash}|-b| e nel caso fosse necessario rimuoverlo basterà usare 
l'opzione \mintinline{bash}|-d|

Successivamente si avvia la macchina virtuale con \mint{bash}|$ ./qvm -x| 
oppure con \mint{bash}|$ ./qvm -n| rispettivamente a seconda che si 
utilizzi o non si utilizzi l'interfaccia grafica sulla macchina guest.

Infine, per collegarsi via SSH si deve usare l'opzione \mintinline{bash}|-a|.
...
aggiungere il file \texttt{.install} che si occupa degli \emph{hook}, cioè 
delle azioni pre-post installazione e rimozione. Infine sono stati 
aggiunti i file per \emph{Systemd} e gli script per avviare i daemon.
...
scaricati da Internet nel momento della creazione dei pacchetti quindi può 
succedere che qualche componente non sia raggiungibile. Se questo è il caso 
allora l'installazione o qualche altra funzionalità fallirebbe. In particolare, 
\texttt{rserve-sandbox-docker-bin} 
...
Per gestire questa situazione complessa ogni distribuzione ha un 
...
da Ubuntu, chiamata \emph{Trisquel}~\cite{trisquel}. In questo caso, abilitando 
i PPA di SWI Prolog~\cite{SWIPrologPPA}, si è riusciti ad installare tutto con 
minime difficoltà. Tuttavia è presente qualche problema che inibisce l'accesso 
ad Rserve da SWISH.
...
\mintinline{bash}|$ diff coinmsw.pl coinmsw_R.pl|
